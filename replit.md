...
-   **SimpleBuilder — 3-step flow** (Apr 2026): `SimpleBuilder.tsx` uses a strict 3-step wizard: Step 1 = yard size tiles + scope toggle (full property / front yard only, "Save ~30%"); Step 2 = plan cards (Standard / Premium / Executive) — tapping Standard or Premium auto-advances after 180 ms; tapping Executive shows an inline Executive+ toggle (+$99/mo) on the card itself before user taps Continue; Step 3 = contact form (name*, phone*, email optional, contact preference radio). Submit fires on step 3. Progress bar shows 3 dots. Seasonal touches appear post-submit on the confirmation screen as an optional collapsible accordion ("Anything to add to your yard plan?") — lead is already captured, touches are for the follow-up call discussion only. GHL webhook + Resend payload shape unchanged (touches default to empty array).
