...
0.  **Home V2 + 3-Step SimpleBuilder (April 2026)**: The root route `/` now serves `pages/home-v2.tsx` — a single-scroll, mobile-first marketing page framed around the "90-Day Yard Reset". It embeds `components/SimpleBuilder.tsx`, a stripped-down 3-step plan builder (Yard Size → Patrol Level → Quick Custom Touches) that hides credit/upgrade jargon from customers but still maps "touches" to the existing `basicAddons`/`premiumAddons` arrays so the lead payload, GHL webhook, Resend email, and `leads` PostgreSQL table all stay compatible — no schema change. The classic landing page is preserved at `/legacy`; `/full` and `/quote-wizard` continue to work for back-compat. `GhlChatWidget` now renders a guaranteed-visible 44×44 close button that persists dismissal in `localStorage['ghl_chat_dismissed']` so the popup never auto-reopens after the user closes it.
...
17. **Unified Terminology**: Plan cards say "Includes X upgrades" with breakdown below (e.g. "3 Basic + 1 Premium"). Conversion text says "Trade 2 Basic upgrades for 1 Premium upgrade anytime". CTAs consistently say "See My Instant Price" (primary) and "Design Your Lawn Plan" (secondary). Wizard header says "See Your Instant Price". Submit button says "See My Instant Price".
18. **Plan Badges**: Premium cards show "Most Popular" badge, Executive cards show "Best Value" badge. Purely presentational. **25-Year Birthday Bonus** badges on each plan card with "Celebrating 25 years of Lawn Trooper" tagline: Basic/Premium get +1 upgrade, Executive gets +1 Premium upgrade.
...
34. **Upgrade Flexibility Section**: Both wizards show a "Flexible Upgrade System" section below plan cards with expandable "View upgrade examples" accordion showing Basic and Premium upgrade categories. "Trade 2 Basic upgrades for 1 Premium upgrade anytime" text always visible.
35. **Simplified Plan Cards**: Plan cards show "Includes X upgrades" with type breakdown below (e.g. "3 Basic + 1 Premium"). No "Swap in next step" text.
36. **Softer Upgrade Instructions**: Upgrade step says "Choose Your Upgrades" instead of "Pick Your Bundled Upgrades".
40. **Mobile Comparison Cards**: On mobile (<md), the horizontal scrolling comparison table is replaced with vertically stacked comparison cards. Each card shows plan name, care level, mowing frequency, lawn treatments, total upgrades included with breakdown, and 25-Year Birthday Bonus. Desktop keeps the full comparison table.
37. **Mulch Wording**: Mulch upgrade is called "Seasonal Mulch Refresh" (not "Basic Mulch Install (Up to 4 Yards)"). No hard quantity limits in customer-facing text.
38. **Commitment Savings**: The commitment bonus section is labeled "Commitment Savings" (not "Commitment Bonus"), clearly separated from the Birthday Bonus promotion. Two distinct promotions: Birthday Bonus (extra upgrade) and Commitment Savings (free months).
39. **Trust Messaging**: "Licensed • Insured • Satisfaction Guaranteed" shown below the trust line in wizard plan steps.
