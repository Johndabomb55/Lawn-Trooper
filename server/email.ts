...
async function getCredentials() {
  ...
  if (envApiKey) {
    return {
      apiKey: envApiKey,
      fromEmail:
        process.env.RESEND_FROM_EMAIL || "Lawn Trooper <onboarding@resend.dev>",
    };
  }
  ...
  if (!hostname) {
    throw new Error(
      "Email provider credentials not found (set RESEND_API_KEY or configure Replit Resend connector)",
    );
  }
  ...
  if (!connectionSettings || !connectionSettings.settings.api_key) {
    throw new Error(
      "Resend not connected - please configure Resend in your Replit integrations",
    );
  }
  ...
  return {
    ...
    fromEmail:
      ...
      "Lawn Trooper <onboarding@resend.dev>",
  };
}
...
async function getResendClient() {
  ...
  console.log(
    "Using Resend API key from connector (starts with):",
...
const ADDON_LABELS: Record<string, string> = {
  ...
  mulch_delivery_install_2yards:
    "Mulch Delivery + Installation (Up to 3 Yards)",
  pine_straw_delivery_install_3yards:
  ...
  mulch_delivery_install_over2yards:
    "Mulch Delivery + Installation (Up to 8 Yards)",
  pine_straw_delivery_install_over3yards:
    "Pine Straw Delivery + Installation (Up to 10 Yards)",
...
export async function sendQuoteEmails(data: QuoteRequestData) {
  ...
  try {
    ...
    console.warn(
      "[email] Could not initialize Resend:",
...
export async function sendLeadEmails(data: LeadEmailData) {
  ...
  try {
    ...
    console.warn(
      "[email] Could not initialize Resend for lead emails:",
      (error as Error).message,
    );
    console.warn("[email] Returning degraded success — lead is still captured");
    return {
  ...
  const termDisplay =
    data.term === "1-year"
      ? "1-Year Subscription"
      : data.term === "2-year"
        ? "2-Year Subscription (Price Lock)"
        : "Month-to-Month";
