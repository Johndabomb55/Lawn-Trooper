...
function buildPlansFromConfig(): PlanDefinition[] {
  const uiOverrides: Record<string, { oldPrice: number; keyStats: Array<{ label: string; value: string }>; features: string[]; description: string }> = {
    basic: {
      ...
      description: "Includes 3 Standard-only upgrade credits",
      features: [
        ...
        "<span class='font-bold text-primary'>Includes 3 Standard-only upgrade credits</span>",
    ...
    premium: {
      ...
      features: [
        ...
        "<span class='text-xs text-muted-foreground'>2 Standard credits = 1 Premium upgrade</span>"
      ]
    },
    executive: {
      oldPrice: 469,
      ...
      features: [
        ...
        "Flower bed weed control (included)",
        "<span class='font-bold text-accent'>Includes 9 upgrade credits (Standard = 1, Premium = 2)</span>",
  ...
  return PLAN_CONFIGS.map((c) => {
    ...
    const allowanceLabel = `${totalCredits} upgrade credits (Standard = 1, Premium = 2)`;
    return {
      id: c.id as PlanId,
      name: c.name,
      price: c.basePrice,
      ...
      allowanceLabel,
      swapLabel: c.allowConversion ? "2 Standard credits = 1 Premium upgrade" : undefined,
    };
  });
}
